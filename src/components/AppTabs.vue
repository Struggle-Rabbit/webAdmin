<template>
  <div class="app-tabs flex items-center h-[--tabs-height] bg-[--header-bg] px-2 sm:px-3 border-b border-[--border-color] select-none">
    <div class="flex-1 flex items-center overflow-x-auto no-scrollbar gap-1 sm:gap-1.5" ref="scrollRef">
      <div
        v-for="tab in tabsStore.tabList"
        :key="tab.name"
        class="tab-item group flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 rounded-md cursor-pointer transition-all duration-200 border border-transparent"
        :class="[
      ...
      >
        ...
        <el-icon 
          v-if="tab.closable" 
          class="tab-close rounded-full p-0.5 opacity-60 hover:opacity-100 hover:bg-white/20 transition-all hidden sm:block" 
          :size="12" 
    ...
    <el-dropdown trigger="click" @command="handleContextCommand" class="hidden sm:block">
